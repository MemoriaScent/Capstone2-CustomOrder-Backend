...
export class DeffuserController {
    ...
    //일반 디퓨저 추가
    @Post('')
    @ApiOperation({summary:'일반 디퓨저 정보 추가', description:'새로운 일반 디퓨저 상품 정보 추가'})
    @ApiResponse({status:201,description:'새로운 디퓨저의 상품 정보를 추가했습니다.'})
    @ApiResponse({status:404,description:'새로운 디퓨저의 상품 정보를 추가하지 못했습니다.'})
    async addDeffuser(@Body() deffuserDto: AddDeffuserRequest, @Res() response: Response){
        ...
        if(res.status===201){
            return response.status(201).json({ message: '새로운 디퓨저의 상품 정보를 추가했습니다.'})
        }

        throw new NotFoundException('새로운 디퓨저 상품 정보를 추가하지 못했습니다.')
    }
    ...
    @ApiResponse({status:404,description:'저장된 디퓨저 정보를 불러오지 못했습니다.'})
    async getDeffuser(){
    ...
    //커스텀 디퓨저 추가
    @Post('/custom')
    @ApiOperation({summary:'커스텀 디퓨저 정보 추가', description:'새로운 커스텀 디퓨저 상품 정보 추가'})
    @ApiResponse({status:201,description:'새로운 커스텀 디퓨저 상품 정보를 추가했습니다.'})
    @ApiResponse({status:404,description:'새로운 커스텀 디퓨저 상품 정보를 추가하지 못했습니다.'})
    async addCustomDeffuser(@Body() customDeffuserDto: AddCustomDeffuserRequest, @Res() response: Response){
        ...
        if(res.status===201){
            return response.status(201).json({ message: '새로운 커스텀 디퓨저 상품 정보를 추가했습니다.'})
        }

        throw new NotFoundException('새로운 커스텀 디퓨저 상품 정보를 추가하지 못했습니다.')
    }

    //커스텀 디퓨저 정보
    @Get('/custom')
    @ApiOperation({summary:'커스텀 디퓨저 정보 가져오기', description:'모든 커스텀 디퓨저 상품 정보를 가져옵니다.' })
    @ApiResponse({status:201,description:'저장된 커스텀 디퓨저 상품 정보를 불러옵니다.'})
    @ApiResponse({status:404,description:'저장된 커스텀 디퓨저 상품 정보를 불러오지 못했습니다.'})
